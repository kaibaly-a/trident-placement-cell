...
public interface StudentRepository extends JpaRepository<Student, String> {
        ...
        @Query(value = "SELECT * FROM STUDENT " +
                        ...
                        "ORDER BY REGDNO ASC", nativeQuery = true)
        List<Student> findFinalYearStudents();

        /**
         * Optimized: Find eligible students based on CGPA requirement AND specific
         * branches.
        ...
        /**
         * Optimized: Find eligible students based on CGPA for all branches (when drive
         * has no branch restrictions).
         */
        @Query(value = "SELECT s.* FROM STUDENT s " +
                        "JOIN student_cgpa c ON s.REGDNO = c.regdno " +
                        "WHERE (EXTRACT(YEAR FROM SYSDATE) - TO_NUMBER(s.ADMISSIONYEAR)) >= 3 " +
        ...
        /**
         * Optimized: Find eligible students explicitly based on course, degree YOP,
         * branches, and CGPA requirement.
         */
        @Query(value = "SELECT s.* FROM STUDENT s " +
                        "JOIN student_cgpa c ON s.REGDNO = c.regdno " +
                        ...
                        "AND UPPER(TRIM(s.BRANCH_CODE)) IN (:branches) " +
                        "ORDER BY s.REGDNO ASC", nativeQuery = true)
        List<Student> findEligibleStudentsWithCourseAndYop(
                        @Param("minCgpa") java.math.BigDecimal minCgpa,
